...
config_file_name = "./configs/CraftPotential2.json"
if args.config != None:
...
V_dense = np.array([PotentialBarriers.CraftPotential2(x, V_x0, V0) for x in x_dense])
...
def main():
    ...
    try:
        ...
        f = r"../video/craft_potential2_" + date + r"_.mp4" 
        writervideo = animation.FFMpegWriter(fps=fps) 
        ani.save(f, writer=writervideo)

        exec_time = datetime.now() - start_time
        log_file = open('info.log', 'a')
        log_file.write('CraftPotential2.py exec time:\n')
