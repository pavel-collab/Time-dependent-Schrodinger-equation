...
parser.add_argument("-d", type=float, help="set a width of potential pit")
...
config_file_name = "./configs/CraftPotential1.json"
if args.config != None:
...
V_dense = np.array([PotentialBarriers.CraftPotential1(x, V_x0, d, V0) for x in x_dense])
...
def main():
    ...
    try:
        ...
        f = r"../video/craft_potential1_" + date + r"_.mp4" 
        writervideo = animation.FFMpegWriter(fps=fps) 
        ani.save(f, writer=writervideo)

        exec_time = datetime.now() - start_time
        log_file = open('info.log', 'a')
        log_file.write('CraftPotential1.py exec time:\n')
